    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        body: expect.stringContaining(
          "Products: Herbal Extracts (Food Ingredients / Nutraceutical Ingredients): Ashwagandha Extract",
        ),
      }),
    );
  });

  it("allows adding a custom product requirement", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          message: "Thank you. Our team will get back to you shortly.",
    ...
    await userEvent.click(screen.getByRole("button", { name: /search and select products/i }));
    await userEvent.type(
      screen.getByPlaceholderText(/add product name or requirement/i),
