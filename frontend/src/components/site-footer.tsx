...
export function SiteFooter() {
  return (
    <footer className="mt-24 border-t border-[var(--line)] bg-[var(--green-950)] text-white">
      <div className="section-shell grid gap-10 py-14 md:grid-cols-[1.1fr_0.8fr_1fr]">
        <div>
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            {certificationStrip.map((item) => (
              <span key={item} className="border border-white/18 px-3 py-2 text-xs text-white/78">
                {item}
              </span>
            ))}
          </div>
